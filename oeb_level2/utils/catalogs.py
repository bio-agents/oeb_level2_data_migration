...
def match_metric_from_label(logger: "Union[logging.Logger, ModuleType]", metrics_graphql: "Sequence[Mapping[str, Any]]", community_prefix: "str", metrics_label: "str", challenge_id: "str", challenge_acronym: "str", challenge_assessment_metrics_d: "Mapping[str, Mapping[str, str]]", dataset_id: "Optional[str]" = None) -> "Optional[MetricsTrio]":
    ...
    if mmi is not None:
        return OpenEBenchUtils.getMetricsTrioFromMetric(mmi, community_prefix, agent_id)
    else:
        return None
...
class IndexedDatasets:
    ...
    def index_dataset(self, raw_dataset: "Mapping[str, Any]") -> "Optional[DatasetValidationSchema]":
        ...
        if is_assessment or is_aggregation:
            ...
            d_on_agent_id = d_on.get("agent_id")
                ...
                for m_cat in self.d_categories:
                    for m in m_cat.get("metrics", []):
                        if m.get("metrics_id") == d_on_metrics_id:
                            potential_agent_id = m.get("agent_id")
                            
                            if potential_agent_id == d_on_agent_id:
                                m_matched = d_on_metrics_id
                                break
                            elif m_agent_should is not None:
                                ...
                                m_agent_should = potential_agent_id
                    if m_matched is not None:
                        break
            
            if m_matched is None:
                if m_agent_should is None:
                    self.logger.error(f"{self.type.value.capitalize()} dataset {index_id} ({'in database' if d_pos is None else 'to be submitted'}) from challenge {', '.join(raw_dataset['challenge_ids'])} depends on metric {d_on_metrics_id} implemented by {d_on_agent_id}, which are not registered as valid {self.type.value} challenge metrics. Fix it")
                else:
                    self.logger.error(f"{self.type.value.capitalize()} dataset {index_id} ({'in database' if d_pos is None else 'to be submitted'}) from challenge {', '.join(raw_dataset['challenge_ids'])} matched metric {d_on_metrics_id}, but mismatched implementation ({d_on_agent_id} instead of {m_agent_should}). Fix it")
        ...
        if isinstance(o_datalink, dict):
            the_error, fetched_payload = self.admin_agents.fetchInlineDataFromDatalink(index_id, o_datalink, discard_unvalidable=not is_assessment and not is_aggregation, override_cache=self.override_cache)
...
class IndexedTestActions:
    ...
    def index_test_action(self, raw_test_action: "Mapping[str, Any]") -> "Optional[IndexedTestActions]":
        ...
        # all the outgoing datasets should depend on this agent_id
        transforming_agent_id = raw_test_action.get("agent_id")
        ...
        for involved_d in raw_test_action.get("involved_datasets", []):
            ...
            if d_role == "incoming":
                ...
                if candidate_d is not None:
                    ...
                    o_agent_id = candidate_d.get("depends_on", {}).get("agent_id")
                    if o_agent_id != transforming_agent_id:
                        self.logger.error(f"Entry {raw_test_action['_id']} (type {self.action_type}) reflects transformation due agent {transforming_agent_id}, but {d_role} dataset {candidate_d_id} depends on {o_agent_id}. Fix it")
                        should_fail = True
