...
if TYPE_CHECKING:
    ...
    from oebagents.auth import OEBCredentials
    
    from oebagents.fetch import IdAndOrigId
...
class OpenEBenchUtils():
    ...
    # function to pull a github repo obtained from https://github.com/inab/vre-process_nextflow-executor/blob/master/agent/VRE_NF.py
    ...
    def doMaterializeRepo(self, git_uri: "str", git_tag: "str") -> "str":
        ...
        return cast("str", self.admin_agents.schemas_manager.checkoutSchemas(
...
"""
        ...
        try:
            response = self.admin_agents.query_graphql(
                query,
                variables=variables,
    ...
    def _setup_oeb_validator(self, data_model_source: "Union[str, Tuple[str, Sequence[SchemaHashEntry]]]") -> "None":
        ...
        self.concept_ids_map = self.admin_agents.fetchIdsAndOrigIds(override_cache=self.override_cache)
        self.schema_validators, self.schema_prefix = self.admin_agents.schemas_manager._setupValidator(
            data_model_source,
    ...
    def load_schemas_from_server(self) -> "Mapping[str, str]":
        if self.schema_validators is None:
            ...
            data_model_in_memory = self.admin_agents.schemas_manager.checkoutSchemas(fetchFromREST=True, override_cache=self.override_cache)
    ...
    def fetchStagedAndSandboxData(self, dataType: "str", filtering_keys: "Mapping[str, Union[Sequence[str], Set[str]]]" = {}) -> "Iterator[Mapping[str, Any]]":
        ...
        for flavor in (FLAVOR_SANDBOX, FLAVOR_STAGED):
            ...
            for d in self.admin_agents.iterateConceptEntries(
                dataType,
                flavor=flavor,
...
"""
                ...
                for comb in iteragents.product(*varvals):
                    variables = {
                        k: v
    ...
    def fetchStagedEntry(self, dataType: "str", the_id: "str") -> "Mapping[str, Any]":
        for fetched_data_type, datares_raw in self.admin_agents.fetchEntriesFromIds(
            [the_id],
    ...
    def check_min_dataset_collisions(self, input_min_datasets: "Sequence[Mapping[str, Any]]", ch_id_to_label_and_sep: "Mapping[str, ChallengeLabelAndSep]") -> "Sequence[Tuple[str, Optional[str], str]]":
        ...
        for tp_id in self.admin_agents.getFetchedConceptIdsMap(override_cache=self.override_cache)["Dataset"]:
...
#                self.gen_expected_assessment_original_id(o_dataset, community_prefix, bench_event_prefix_et_al, "", "")
            ...
            if (not isinstance(default_schema_url, bool) or default_schema_url) and isinstance(o_datalink, dict):
                the_error, fetched_inline_data = self.admin_agents.fetchInlineDataFromDatalink(
                    o_id,
                    o_dataset["datalink"],
    ...
    @staticmethod
    def getMetricsTrioFromMetric(mmi: "Mapping[str, Any]", community_prefix: "str", agent_id: "Optional[str]") -> "MetricsTrio":
    ...
    def getMetricsTrioFromMetricsId(self, metrics_id: "str", community_prefix: "str", agent_id: "Optional[str]") -> "MetricsTrio":
        mmi = self.fetchStagedEntry("Metrics", metrics_id)
        return self.getMetricsTrioFromMetric(mmi, community_prefix, agent_id)
