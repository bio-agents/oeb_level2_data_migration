...
def prepare_participants_reanalysis(
    ...
    for uri, input_file in uri2path.items():
        ...
        for p_id in p_ids:
            ...
            # Using first valid challenge to get the agent details
            # Should a check about all of them using the same agent be added?
            agent_link: "Optional[str]" = None
            workflow_agent_id: "Optional[str]" = None
            for challenge_pair in p_tuple.challenge_pairs:
                ...
                for m_cat in c_entry.get("metrics_categories", []):
                    if m_cat.get("category") == AGGREGATION_CATEGORY_LABEL:
                        for m_agent in m_cat.get("metrics", []):
                            workflow_agent_id = m_agent.get("agent_id")
                            if workflow_agent_id is None:
                                logging.fatal(f"Fix challenge {c_label}")
                                sys.exit(3)
                            
                            agent = migration_utils.fetchStagedEntry("Agent", workflow_agent_id)
                            for agent_access in agent.get("agent_access", []):
                                if agent_access.get("agent_access_type") == "other":
                                    agent_link = agent_access.get("link")
                                    break
            ...
            if agent_link is not None:
                ...
                parsed_agent_link = urllib.parse.urlparse(agent_link)
                splitted_path = parsed_agent_link.path.split("/")
                ...
                parsed_agent_link_trimmed = parsed_agent_link._replace(path="/".join(splitted_path[0:3]))
                nextflow_repo_uri = urllib.parse.urlunparse(parsed_agent_link_trimmed)
                ...
                logging.fatal(f"FIXME: challenges {challenge_labels_spc} agent {workflow_agent_id}")
                sys.exit(3)
            
            assert workflow_agent_id is not None
...
def reanalyse_challenges(
    ...
    for job_i, job_to_perform in enumerate(jobs_to_perform, 1):
        ...
        config_level2 = {
            ...
            "agent_mapping": [
                {
                    ...
                    "agent_id": job_to_perform.p_tuple.p_config.agent_id,
                    "data_version": 1,
            ...
            "workflow_oeb_id": job_to_perform.workflow_agent_id,
        }
