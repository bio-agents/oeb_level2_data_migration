...
"""
#########################################################
	    VRE Level 2 to OpenEBench migration agent 
		Authors:
...
from oebagents.fetch import OEBFetcher
from rfc3339_validator import validate_rfc3339  # type: ignore[import]

from oebagents.uploader import(
    PayloadMode,
)
...
def validate_transform_and_push(
    ...
    try:
        ...
        # Agent mapping contains the correspondence from
        # participant label to official OpenEBench local id
        agent_mapping: "MutableMapping[Optional[str], ParticipantConfig]" = {}
        agent_mapping_list = config_params.get("agent_mapping")
        if agent_mapping_list is None:
            # Old school assumed there were only a agent_id declaration
            # so a "blind mapping" is only possible
            agent_mapping_old = cast("_ParticipantElements", config_params)
            agent_id = agent_mapping_old["agent_id"]
            ...
            agent_mapping[None] = ParticipantConfig(
                agent_id=agent_id,
                data_version=str(agent_mapping_old["data_version"]),
                data_contacts=agent_mapping_old["data_contacts"],
            ...
            for agent_mapping_e in agent_mapping_list:
                participant_label = agent_mapping_e["participant_id"]
                assert participant_label is not None

                participant_file = agent_mapping_e.get("participant_file", old_file_location)
                ...
                agent_mapping[participant_label] = ParticipantConfig(
                    agent_id=agent_mapping_e["agent_id"],
                    data_version=str(agent_mapping_e["data_version"]),
                    data_contacts=agent_mapping_e["data_contacts"],
                    participant_label=participant_label,
                    participant_file=participant_file,
                    exclude=agent_mapping_e.get("exclude", False),
                )
    ...
    for i_dataset, dataset in enumerate(data):
        ...
        if dataset_type == OEBDatasetType.Participant.value:
            ...
            if agent_id is not None:
                ...
                elif None in agent_mapping:
                    # Fixed "old" default agent mapping
                    agent_mapping[None].participant_label = participant_label
                    agent_mapping[participant_label] = agent_mapping[None]
                    del agent_mapping[None]
            ...
            for challenge in challenges:
                ...
                if agent_id is not None and p_in_chall[participant_label] > 1:
    ...
    # Early check over the agents
    logging.info("-> Fetching the list of recorded agents")
    allAgents = list(migration_utils.fetchStagedAndSandboxData('Agent'))
    logging.info("-> Early checking in the participant agents")
    unique_participant_agent_ids = set(map(lambda pc: pc.agent_id, agent_mapping.values()))
    participant_agents = list(OEBFetcher.filter_by(allAgents, {"_id": unique_participant_agent_ids}))
    participant_agent_ids = set(map(lambda pt: cast("str", pt["_id"]), participant_agents))
    if participant_agent_ids < unique_participant_agent_ids:
        logging.fatal(
            f"Agent ids {', '.join(unique_participant_agent_ids.difference(participant_agent_ids))} could not be found in OEB. Maybe you are using the wrong instance (development instead of production) or the agent ids have a typo. Otherwise, please contact OpenEBench support for information about how to register these agents")
        sys.exit(2)
    ...
    # query remote OEB database to get offical ids from associated challenges, agents and contacts
    ...
    for p_config in agent_mapping.values():
    ...
    valid_aggregation_tuples = aggregations_builder.build_aggregation_datasets(
        ...
        putative_workflow_agent_id=workflow_id,
