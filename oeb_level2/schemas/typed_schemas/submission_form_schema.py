...
class ConfigParams(TypedDict, total=False):
    ...
    agent_id: "ParticipantAgent"
    """
    Participant Agent.

    The id of the agent used to generate the dataset. Should be selected by uploader, using API

    pattern: ^OEBT[0-9]{3}[A-Z0-9]{7}$
    """

    agent_mapping: List["_ParticipantElements"]
    """
    List of agents.
    ...
    The id of the workflow (as a agent) used to compute the assessment metrics. Should be associated to VRE agent
...
class _ParticipantElements(TypedDict, total=False):
    agent_id: Required["ParticipantAgent"]
    """
    Participant Agent.
    ...
    participant_id: str
    """
    The id / name of the agent which generated this dataset.
