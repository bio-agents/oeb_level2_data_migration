...
from oebagents.fetch import FetchedInlineData
...
class AggregationValidator():
    ...
    def __init__(self, schemaMappings: "Mapping[str, str]", migration_utils: "OpenEBenchUtils"):
        ...
        self.admin_agents = migration_utils.admin_agents
            ...
            if len(x_dataset_ids) > 1:
                ...
                for x_dataset_id in x_dataset_ids:
                    ...
                    if len(raw_x_datasets) > 0:
                        # Gather all the agents
                        x_agent_id: "Optional[str]" = raw_x_datasets[0].get("depends_on", {}).get("agent_id")
                        if x_agent_id is not None:
                            s_agent_id.add(x_agent_id)
                ...
                if len(s_agent_id) > 1:
                    logger.fatal(f"{pairs_desc.value.capitalize()} label {x_label} datasets depend on more than one agent: {', '.join(s_agent_id)}")
                    failed_x_dataset = True
                else:
                    ...
                    if metrics_trios:
                        ...
                        if isinstance(fetched_inline_data, FetchedInlineData):
                            ...
                            for i_trio, metrics_trio in enumerate(metrics_trios):
                                ...
                                if metrics_trio.representation_hints is not None:
                                    metrics_limits = metrics_trio.representation_hints.get("limits")
                            ...
                            if not rebuild_agg:
                                ...
                                if len(challenge_participants) != len(raw_challenge_participants):
                                    ...
                                    if vis_type == "box-plot":
                                        ...
                                        s_new_challenge_participants = sorted(cast("Sequence[ScatterData]", challenge_participants), key=lambda cp: cp["agent_id"])
                                        s_raw_challenge_participants = sorted(cast("Sequence[ScatterData]", raw_challenge_participants), key=lambda cp: cp["agent_id"])
                                    elif vis_type == "bar-plot":
                                        s_new_challenge_participants = sorted(cast("Sequence[BarData]", challenge_participants), key=lambda cp: cp["agent_id"])
                                        s_raw_challenge_participants = sorted(cast("Sequence[BarData]", raw_challenge_participants), key=lambda cp: cp["agent_id"])
                                    else:
...
class AggregationBuilder():
    ...
    def build_aggregation_datasets(
        ...
        putative_workflow_agent_id: "Optional[str]",
        bench_event_prefix_et_al: "BenchmarkingEventPrefixEtAl",
        community_prefix: "str",
        ...
        for min_dataset in min_aggregation_datasets:
            ...
            workflow_agent_id = putative_workflow_agent_id
            ...
            for challenge_label in min_dataset["challenge_ids"]:
                    ...
                    if m_cat == AGGREGATION_CATEGORY_LABEL:
                        if not wmi_was_set:
                            ...
                            for metric_decl in metrics_category.get("metrics", []):
                                metric_agent_id = metric_decl.get("agent_id")
                                mmi = self.migration_utils.fetchStagedEntry('Metrics', metric_decl["metrics_id"])
                                metrics_trio = self.migration_utils.getMetricsTrioFromMetric(mmi, idx_agg.d_catalog.community_prefix, metric_agent_id)
                                if the_agg_metric_label is not None:
                                    ...
                                    break
                                elif metric_agent_id is None or putative_workflow_agent_id is None or metric_agent_id == putative_workflow_agent_id:
                            ...
                            if len(workflow_metrics) > 0:
                                ...
                                if metrics_trio.agent_id is not None and metrics_trio.agent_id != putative_workflow_agent_id:
                                    self.logger.warning(f"Proposed agent {putative_workflow_agent_id} does not match in the metrics declaration at the challenge. Using instead {metrics_trio.agent_id}")
                                    workflow_agent_id = metrics_trio.agent_id
                                wmi_was_set = True
                ...
                if not wmi_was_set:
                    self.logger.critical(f"In challenge {idx_agg.challenge['_id']}, unable to set/guess the metrics id from workflow agent id {workflow_agent_id}")
                            ...
                            for met_dataset in met_datasets:
                                ...
                                if mini_entry is None:
                                    if vis_type == "2D-plot":
                                        ...
                                        d_on_agent_id = m_depends_on.get("agent_id")
                                        ass_trio = self.migration_utils.getMetricsTrioFromMetricsId(d_on_metrics_id, idat_ass.community_prefix, d_on_agent_id)
                                        mini_entry_s = {
                                            "label": par_label,
    ...
    def build_aggregation_events(
        ...
        for agt in valid_aggregation_tuples:
            ...
            # add id of agent for the test event
            event["agent_id"] = dataset["depends_on"]["agent_id"]
